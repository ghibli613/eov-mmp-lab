...
from vlm.backbones.clip.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class TemporalDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        # self.transformer = nn.TransformerEncoderLayer(d_model=2048, nhead=8, batch_first=True)
        # self.pos_embeddings = nn.Embedding(40, 2048)
        # self.relEmb = nn.Linear(2048, 2048, bias=False)
        ...
        self.transformer = nn.TransformerEncoderLayer(d_model=768, nhead=8, batch_first=True)
        self.pos_embeddings = nn.Embedding(40, 768)
        self.relEmb = nn.Linear(768, 768, bias=False)
    ...
    def forward(self, x_clip, seq_lens):
        ...
        # seq_lens = seq_lens.unsqueeze(-1).repeat(1, 4).view(-1).unsqueeze(-1).expand(-1, slen)
        # pos_ids = torch.arange(slen, dtype=torch.long).unsqueeze(0).expand(bs*4, -1).cuda()
        # padding_mask = pos_ids > seq_lens

        x_clip = x_clip + self._gen_pos_embeddings(x_clip.shape[0], x_clip.shape[1])
        x_clip = self.transformer(x_clip)#, src_key_padding_mask=padding_mask)
        x_clip = x_clip.reshape(bs, 4, slen, -1).permute(0, 2, 1, 3)

        x_int = self.intHead(x_clip.reshape(bs, slen, -1)).squeeze(-1)
        # x_clip = x_clip + x_origin
...
class SpatialDecoder(nn.Module):
    def __init__(self):
        ...
        #     nn.TransformerEncoderLayer(d_model=512, nhead=8, batch_first=True)
        # )
        self.role_embeddings = nn.Embedding(4, 768)
        self.relEmb = nn.Linear(768, 768, bias=False)
        self.objEmb = nn.Linear(768, 768, bias=False)
    ...
    def forward(self, x_img, x_box=None, x_pos=None):
        ...
        # x_rel = F.normalize(self.relEmb(x), dim=-1)
        x_rel = x
...
class PromptLearner(nn.Module):
    ...
    def forward(self, im_features):
        ...
        bias = self.meta_net(torch.mean(im_features,dim=1))  # (batch, ctx_dim)
        bias = bias.unsqueeze(1)           # (batch, 1, ctx_dim)
        ctx = ctx.unsqueeze(0)             # (1, n_ctx, ctx_dim)
        bias_reshape = bias.expand(1,16,768)
...
class ObjectTextEncoder(nn.Module):
    def __init__(self, text_encoder='clip', cls_split_info_path=paths.OBJ_SPLIT_INFO):
        super().__init__()
...
class PredicateTextEncoder(nn.Module):
    def __init__(self, text_encoder='clip', cls_split_info_path=paths.PRED_SPLIT_INFO):
        super().__init__()
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        cls_split_info = json.load(open(paths.PRED_SPLIT_INFO, 'r'))
        self.id2cls = cls_split_info['id2cls']
        self.cls2id = cls_split_info['cls2id']
        self.cls2split = cls_split_info['cls2split']
        self.base_pids = [self.cls2id[cls] for cls in self.cls2id if self.cls2split[cls] == 'base']
    ...
    def split_text_embeddings(self, split, pre_classifier_weights):
        ...
        pre_obj_text_embeddings = self.pre_text_encoder.split_classifier_weights(
            ...
            split=split).detach()
        pre_uni_text_embeddings = self.pre_text_encoder.split_classifier_weights(
        ...
        pre_text_embeddings = torch.cat([pre_sbj_text_embeddings, pre_obj_text_embeddings, pre_uni_text_embeddings, pre_rel_text_embeddings], dim=-1) / 2
