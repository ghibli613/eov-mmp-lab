...
import models.tracking.aflink.config as cfg
from models.tracking.aflink.train import train
from models.tracking.aflink.dataset import LinkData
from models.tracking.aflink.model import PostLinker
INFINITY = 1e5

class AFLink:
    def __init__(self, results, model, dataset, thrT: tuple, thrS: int, thrP: float):
        self.thrP = thrP          # 预测阈值
        self.thrT = thrT          # 时域阈值
        self.thrS = thrS          # 空域阈值
        self.model = model        # 预测模型
        self.dataset = dataset    # 数据集类
        self.track = np.array(results)
        self.model.cuda()
        self.model.eval()
...
if __name__ == '__main__':
    print(datetime.now())
    # dir_in = '../output/data/MOT20_StrongSORT'
    dir_in = '../output/data/CenterTrack'
    dir_out = dir_in + '_tmp'
    # dir_out = '../output/data/CenterTrack_AFLink'
