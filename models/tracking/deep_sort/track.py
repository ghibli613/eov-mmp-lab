...
import numpy as np
from models.tracking.deep_sort.kalman_filter import KalmanFilter
...
class Track:
    ...
    def camera_update(self, video, frame, ecc):
        if video in ecc.keys():
            dict_frame_matrix = ecc[video]
            frame = str(int(frame))
    ...
    def update(self, detection,args):
        ...
        """
        self.mean, self.covariance = self.kf.update(args,self.mean, self.covariance, detection.to_xyah(), detection.confidence)

        feature = detection.feature / np.linalg.norm(detection.feature)
        if args.EMA:
            smooth_feat = args.EMA_alpha * self.features[-1] + (1 - args.EMA_alpha) * feature
