...
from utils.eov_utils import vru19_ext_loc_feat, gen_union_bbox, ext_bbox_loc_feat
import copy
...
class FeatExtractor:
    ...
    def load_frames(self, data):
        self.num_frames = len(os.listdir(join(paths.FRAME_DIR, data['video_id'])))
...
def gen_frame_pre_label(frame_id, anno):
    split_path = paths.PRED_SPLIT_INFO
...
def gen_box_label(frame_id, trajs, w, h):
    split_path = paths.OBJ_SPLIT_INFO
    with open(split_path, "r") as f:
        split = json.load(f)
    cls2id = split['cls2id']
    xyxy_boxes = []
...
def gen_feats(video_name, trajectories, split, patch_proj, global_proj,w,h):
    object2id = json.load(open(join(paths.META_DIR, 'object2id.json'),'r'))
    predicate2id = json.load(open(join(paths.META_DIR, 'predicate2id.json'),'r'))
    path = join(paths.ANNO_DIR, split)
