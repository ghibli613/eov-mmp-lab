...
from models.gen_labels import gen_frame_pre_label, gen_box_label, create_mask, compute_iou
from models.gen_labels import format_trajectories, gen_feats, filter, format_trajectories_test, gen_feats_test
import numpy as np
import warnings
...
def _load_ecc(path):
    """Per-video camera-motion (ECC) matrices for deep_sort's camera_update.

    EOV loads these unconditionally but ships neither file, so the import used to
    die here. They are NOT reconstructible from anything in this repo -- they come
    from running ECC image alignment over each video.

    Missing -> empty dict. `Tracker.camera_update` guards with `if video in
    ecc.keys()`, so an empty dict makes camera compensation a no-op: tracking
    still runs, just without motion compensation. That is a real (small) quality
    loss on videos with a moving camera, not a silent equivalence -- hence the
    warning. See docs/PORT_STATUS.md.
    """
    if os.path.exists(path):
        return json.load(open(path))
    # NB: this module calls warnings.filterwarnings("ignore") at import time,
    # so warnings.warn() here would be swallowed. Print so it is actually seen.
    print(f"[WARN] ECC matrices not found at {path}; camera-motion compensation "
          f"disabled. Tracking runs without it.", file=sys.stderr)
    return {}
...
class End2End_Model(nn.Module):
    def __init__(self, args, modelA, modelB, modelC, criterionA, postprocessorA):
        ...
        self.traj = {}
        self.train_ecc = _load_ecc(paths.ECC_TRAIN)
        self.test_ecc = _load_ecc(paths.ECC_TEST)
    ...
    def forward(self, input, sort_model):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                # path = '../dataset/vidvrd/data/test_object_trajectories_e2e.json'
                # ddd = json.load(open(path,'r'))
                # associate_result = ddd[input['video_name'][0]]
                final_results = []
                if len(associate_result) > 0:
                    datasetss = LinkData('', '')
                    linker = AFLink(
