    ...
    def forward(self, xs, up_size):
        ...
        if len(xs) == 2:
            ...
            output = nn.functional.interpolate(
              output, size=up_size, mode="bilinear", align_corners=self.align_corners
            )
...
from third_party.detectron2.layers import Conv2d, get_norm
import math
...
class SimpleFeaturePyramid(nn.Module):
    ...
    def __init__(
        ...
        out_channels,
        scale_factors,
        stride=16, # vit default 16 x 16 patch
        top_block=None,
        norm="LN",
        square_pad=0,
    ):
        ...
        strides = [(stride / scale) for scale in scale_factors]
        print('FPN strides:', strides)
