...
def vru19_ext_loc_feat(sbj_box, obj_box, img_h, img_w):
    ...
    sbj_h = max(sbj_box['ymax'] - sbj_box['ymin'] + 1, 1)
    sbj_w = max(sbj_box['xmax'] - sbj_box['xmin'] + 1, 1)
    obj_h = max(obj_box['ymax'] - obj_box['ymin'] + 1, 1)
    obj_w = max(obj_box['xmax'] - obj_box['xmin'] + 1, 1)
    spatial_feat = [
