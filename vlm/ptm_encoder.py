...
from vlm.backbones import internvideo as InternVideo
from PIL import Image
import numpy as np
import torch
import torch.nn as nn
from torchvision import transforms
from vlm.backbones.internvideo import video_transform
...
class CLIPVisualEncoder(nn.Module):
    ...
    def forward(self, regions):
        ...
        for image_path, bbox in regions:
            ...
            feat = self.visual(image.type(self.dtype))[0].detach().cpu().numpy()[0]
            feats.append(feat)
...
class InternlVisualEncoder(nn.Module):
    def __init__(self, backbone="ViT-B/16", gpu_id=0):
        super().__init__()
        self.device = f"cuda:{gpu_id}" if torch.cuda.is_available() else "cpu"
        self.model = InternVideo.load_model("../model/ckpt/InternVideo-MM-L-14.ckpt").cuda(device=self.device)
